...
namespace Assets.HeroEditor4D.Common.Scripts.CharacterScripts
{
    ...
    public class CharacterAppearance
    {
        ...
        public Color32 HairColor = new(150, 50, 0, 255);
        public Color32 BeardColor = new(150, 50, 0, 255);
        public Color32 EyesColor = new(0, 200, 255, 255);
        public Color32 BodyColor = new(255, 200, 120, 255);
        public Color32 UnderwearColor = new(120, 100, 80, 255);
        ...
        {
            if (character.SpriteCollection.Id != "FantasyHeroes" && character.SpriteCollection.Id != "MilitaryHeroes")
                return; // Not supported yet.

            var hair = Hair.IsEmpty() ? null : character.SpriteCollection.Hair.Single(i => i.Id == Hair);

            character.Hair = hair == null
                ? null
                : character.HairRenderer.GetComponent<SpriteMapping>().FindSprite(hair.Sprites);
            character.HairRenderer.color =
                hair != null && hair.Tags.Contains("NoPaint") ? (Color32)Color.white : HairColor;

            if (character.BeardRenderer)
            {
                var beard = Beard.IsEmpty() ? null : character.SpriteCollection.Beard.Single(i => i.Id == Beard);

                character.Beard = beard == null
                    ? null
                    : character.BeardRenderer.GetComponent<SpriteMapping>().FindSprite(beard.Sprites);
            ...
            if (character.Expressions.Count > 0)
            {
                ...
                {
                    character.Expressions[0].Eyebrows = Eyebrows.IsEmpty()
                        ? null
                        : character.EyebrowsRenderer.GetComponent<SpriteMapping>()
                            .FindSprite(character.SpriteCollection.Eyebrows.FindSprites(Eyebrows));
                    character.Expressions[0].Eyes = character.EyesRenderer.GetComponent<SpriteMapping>()
                        .FindSprite(character.SpriteCollection.Eyes.FindSprites(Eyes));
                    character.Expressions[0].Mouth = character.MouthRenderer.GetComponent<SpriteMapping>()
                        .FindSprite(character.SpriteCollection.Mouth.FindSprites(Mouth));
                }

                foreach (var expression in character.Expressions)
                    if (expression.Name != "Dead")
                        expression.EyesColor = EyesColor;
            }

            if (character.EyesRenderer != null) character.EyesRenderer.color = EyesColor;
            ...
            if (body.Tags.Contains("NoMouth")) character.Expressions.ForEach(i => i.Mouth = null);
