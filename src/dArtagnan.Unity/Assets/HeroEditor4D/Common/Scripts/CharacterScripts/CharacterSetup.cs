    ...
    public partial class Character
    {
        ...
        public void SetBody(ItemSprite item, BodyPart part, Color? color)
        {
            switch (part)
            {
                ...
                case BodyPart.Eyebrows:
                    if (EyebrowsRenderer)
                        Expressions[0].Eyebrows =
                            EyebrowsRenderer.GetComponent<SpriteMapping>().FindSprite(item?.Sprites);
                    break;
                case BodyPart.Eyes:
                    if (EyesRenderer)
                    {
                        Expressions[0].Eyes = EyesRenderer.GetComponent<SpriteMapping>().FindSprite(item?.Sprites);
                        Expressions.Where(i => i.Name != "Dead").ToList()
                            .ForEach(i => i.EyesColor = color ?? EyesRenderer.color);
        ...
        public void Equip(ItemSprite item, EquipmentPart part, Color? color)
        {
            ...
            switch (part)
            {
                ...
                case EquipmentPart.MeleeWeapon1H:
                    ...
                    PrimaryWeaponRenderer.color = color ?? (item != null && item.Tags.Contains("Paint")
                        ? PrimaryWeaponRenderer.color
                        : Color.white);
                ...
                case EquipmentPart.SecondaryMelee1H:
                    ...
                    SecondaryWeaponRenderer.color = color ?? (item != null && item.Tags.Contains("Paint")
                        ? SecondaryWeaponRenderer.color
                        : Color.white);
            ...
            if (part == EquipmentPart.Firearm1H || part == EquipmentPart.Firearm2H) SetFirearmMuzzle(item);
        }
        ...
        private void SetArmorParts(List<SpriteRenderer> renderers, List<Sprite> armor, Color? color)
        {
            ...
            foreach (var r in renderers)
            {
                ...
                var sprite = armor?.SingleOrDefault(j =>
                    mapping.SpriteName == j.name || mapping.SpriteNameFallback.Contains(j.name));

                Armor.RemoveAll(i => i.name == mapping.SpriteName || mapping.SpriteNameFallback.Contains(i.name));

                if (sprite != null) Armor.Add(sprite);
        ...
        private void SetFirearmMuzzle(ItemSprite item)
        {
            ...
            var p = new Vector2(bounds.min.x + mx * (bounds.max.x - bounds.min.x),
                bounds.min.y + my * (bounds.max.y - bounds.min.y));
