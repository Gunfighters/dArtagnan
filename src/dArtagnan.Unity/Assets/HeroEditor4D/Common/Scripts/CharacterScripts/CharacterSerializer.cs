...
namespace Assets.HeroEditor4D.Common.Scripts.CharacterScripts
{
    public partial class Character
    {
        public string ToJson()
        {
            ...
            switch (WeaponType)
            {
                ...
                case WeaponType.Firearm2H:
                    description.Add("PrimaryWeapon",
                        SpriteToString(GetWeaponCollection(WeaponType), PrimaryWeaponRenderer));
                    break;
                case WeaponType.Paired:
                    description.Add("SecondaryWeapon",
                        SpriteToString(SpriteCollection.Firearm1H, SecondaryWeaponRenderer)); // TODO:
                    break;
                case WeaponType.Bow:
            ...
            {
                description.Add($"Expression.{expression.Name}.Eyebrows",
                    SpriteToString(SpriteCollection.Eyebrows, expression.Eyebrows, EyebrowsRenderer.color));
                description.Add($"Expression.{expression.Name}.Eyes",
                    SpriteToString(SpriteCollection.Eyes, expression.Eyes, EyesRenderer.color));
                description.Add($"Expression.{expression.Name}.EyesColor",
                    "#" + ColorUtility.ToHtmlStringRGBA(expression.EyesColor));
                description.Add($"Expression.{expression.Name}.Mouth",
                    SpriteToString(SpriteCollection.Mouth, expression.Mouth, MouthRenderer.color));
            }
        ...
        public void LoadFromJson(string json, bool silent)
        {
            ...
            RestoreFromString(ref Backpack, BackpackRenderer, SpriteCollection.Backpack, description["Backpack"],
                silent);
            ...
            RestoreFromString(ref Earrings, EarringsRenderers, SpriteCollection.Earrings, description["Earrings"],
                silent);
            ...
            foreach (var key in description.Keys)
                if (key.Contains("Expression."))
                {
                    ...
                    {
                        case "Eyebrows":
                            RestoreFromString(ref expression.Eyebrows, EyebrowsRenderer, SpriteCollection.Eyebrows,
                                description[key]);
                            break;
                        case "Eyes":
                            RestoreFromString(ref expression.Eyes, EyesRenderer, SpriteCollection.Eyes,
                                description[key]);
                            break;
                        case "EyesColor":
                            ...
                            break;
                        case "Mouth":
                            RestoreFromString(ref expression.Mouth, MouthRenderer, SpriteCollection.Mouth,
                                description[key]);
                            break;
                        default:
            ...
            {
                var weapon = RestoreFromString(ref PrimaryWeapon, PrimaryWeaponRenderer,
                    GetWeaponCollection(WeaponType), description["PrimaryWeapon"], silent);
            ...
            if (description.ContainsKey("SecondaryWeapon"))
                RestoreFromString(ref SecondaryWeapon, SecondaryWeaponRenderer, SpriteCollection.Firearm1H,
                    description["SecondaryWeapon"], silent);
            ...
            if (description.ContainsKey("Bow"))
                ...
                RestoreFromString(ref CompositeWeapon, CrossbowRenderers, SpriteCollection.Crossbow,
                    description["Crossbow"], silent);
        ...
        private static string SpriteToString(IEnumerable<ItemSprite> collection, Sprite sprite, Color color)
        {
            ...
            if (entry == null) throw new Exception($"Can't find {sprite.name} in SpriteCollection.");
        ...
        private ItemSprite RestoreFromString(ref Sprite sprite, SpriteRenderer renderer,
            IEnumerable<ItemSprite> collection, string serialized, bool silent = false)
        {
            if (renderer == null) return null;
            ...
            if (parts.Length > 1) ColorUtility.TryParseHtmlString("#" + parts[1], out color);
            ...
            {
                case 1:
                    sprite = renderer.sprite = entries[0].Sprites.Count == 1
                        ? entries[0].Sprites[0]
                        : renderer.GetComponent<SpriteMapping>().FindSprite(entries[0].Sprites);
        ...
        private static void RestoreFromString(ref List<Sprite> sprites, List<SpriteRenderer> renderers,
            List<ItemSprite> collection, string serialized, bool silent = false)
        {
            if (string.IsNullOrEmpty(serialized))
            {
