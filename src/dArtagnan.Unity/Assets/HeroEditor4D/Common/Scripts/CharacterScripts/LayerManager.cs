...
namespace Assets.HeroEditor4D.Common.Scripts.CharacterScripts
{
    ...
    public class LayerManager : MonoBehaviour
    {
        ...
        {
            for (var i = 0; i < Sprites.Count; i++) Sprites[i].sortingOrder = 5 * i;
        ...
        public void CopyOrder()
        {
            ...
            foreach (var sprite in CopyTo.Sprites)
                sprite.sortingOrder = Sprites.Single(i =>
                    i.name == sprite.name && GetSpriteRendererPath(i) == GetSpriteRendererPath(sprite)).sortingOrder;
