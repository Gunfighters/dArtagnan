...
namespace Assets.HeroEditor4D.Common.Scripts.Editor
{
    ...
    public class LayerManagerEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Read Sorting Order")) script.GetSpritesBySortingOrder();

            if (GUILayout.Button("Set Sorting Order")) script.SetSpritesBySortingOrder();

            if (GUILayout.Button("Copy order")) script.CopyOrder();
        }
    }
}