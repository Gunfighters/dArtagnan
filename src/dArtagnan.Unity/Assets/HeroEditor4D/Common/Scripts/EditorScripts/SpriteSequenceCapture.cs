...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class SpriteSequenceCapture : MonoBehaviour
    {
        ...
        {
            UpperBodyClipNames = Directory.GetFiles(UpperBodyAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension).ToList();
            LowerBodyClipNames = Directory.GetFiles(LowerBodyAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension).ToList();
            ComplexClipNames = Directory.GetFiles(ComplexAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension).ToList();
            UpperBodyClipNames.AddRange(ComplexClipNames);

            for (var i = 0; i < UpperBodyClipNames.Count; i++)
                if (UpperBodyClipNames[i].EndsWith("U"))
                    UpperBodyClipNames[i] = UpperBodyClipNames[i].Substring(0, UpperBodyClipNames[i].Length - 1);

            for (var i = 0; i < LowerBodyClipNames.Count; i++)
                if (LowerBodyClipNames[i].EndsWith("L"))
                    LowerBodyClipNames[i] = LowerBodyClipNames[i].Substring(0, LowerBodyClipNames[i].Length - 1);
        ...
        public void Start()
        {
            ...
            foreach (var dropdown in new[]
                     {
                         PrimaryAnimationDropdown, SecondaryAnimationDropdown, FrameSizeDropdown, FrameRatioDropdown,
                         ScreenshotIntervalDropdown, ShadowDropdown
                     })
        ...
        public void Load()
        {
            var path = EditorUtility.OpenFilePanel("Open character prefab", "", "prefab");
        ...
        private void ShowFrame(string upperClip, string lowerClip, float normalizedTime)
        {
            ...
            if (Character4D.Animator.IsInTransition(1)) Debug.Log("IsInTransition");
        }

        private IEnumerator CaptureFrames(List<string> upperClips, List<string> lowerClips, int frameSize,
            int frameRatio, float interval)
        {
            ...
            foreach (var upperClip in upperClips)
            {
                foreach (var lowerClip in lowerClips)
                    for (var i = 0; i < frameRatio; i++)
                    {
                        ...
                        var path = death
                            ? $"{Application.dataPath.Replace("/Assets", null)}/SpriteSheets/{direction}/{upperClip}/{i}.png"
                            : $"{Application.dataPath.Replace("/Assets", null)}/SpriteSheets/{direction}/{upperClip}-{lowerClip}/{i}.png";
