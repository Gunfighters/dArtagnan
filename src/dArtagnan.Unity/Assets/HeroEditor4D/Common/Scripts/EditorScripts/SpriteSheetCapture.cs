...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class SpriteSheetCapture : MonoBehaviour
    {
            ...
            bool shadow)
        {
            ...
            var stateHandler = _character.Animator.GetBehaviours<StateHandler>()
                .SingleOrDefault(i => i.Name == "Death");

            if (stateHandler) stateHandler.StateExit.RemoveAllListeners();
            ...
            foreach (var option in options)
            {
                ...
                for (var j = 0; j < frameCount; j++)
                {
                    ...
                    var expressionEvent = clip.events.Where(i =>
                            i.functionName == "SetExpression" && Mathf.Abs(i.time / clip.length - normalizedTime) <=
                            1f / (frameCount - 1))
                        .OrderBy(i => Mathf.Abs(i.time / clip.length - normalizedTime)).FirstOrDefault();

                    if (expressionEvent != null) _character.SetExpression(expressionEvent.stringParameter);
            ...
            if (stateHandler) stateHandler.StateExit.AddListener(() => _character.SetExpression("Default"));

            var texture = CreateSheet(clips, frameSize, frameSize);

            yield return StandaloneFilePicker.SaveFile("Save as sprite sheet", "", "Character", ".png",
                texture.EncodeToPNG(),
        ...
        private IEnumerator ShowFrame(string stateL, string stateU, string stateC, float normalizedTime)
        {
            ...
            if (_character.Animator.IsInTransition(1)) Debug.Log("IsInTransition");
        }
        ...
        private Texture2D CreateSheet(Dictionary<string, List<Texture2D>> clips, int width, int height)
        {
            ...
            foreach (var clip in clips)
                for (var i = 0; i < clip.Value.Count; i++)
                    texture.SetPixels(i * width, clips.Keys.Reverse().ToList().IndexOf(clip.Key) * height, width,
                        height, clip.Value[i].GetPixels());
