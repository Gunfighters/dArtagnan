...
namespace Assets.HeroEditor4D.Common.Scripts.Common
{
    public static class StandaloneFilePicker
    {
        ...
        public static IEnumerator OpenFile(string title, string directory, string extension,
            Action<bool, string, byte[]> callback)
        {
            var path = EditorUtility.OpenFilePanel(title, directory, extension.Replace(".", null));
        ...
        public static IEnumerator SaveFile(string title, string directory, string defaultName, string extension,
            byte[] bytes, Action<bool, string> callback)
        {
            var path = EditorUtility.SaveFilePanel(title, directory, defaultName, extension.Replace(".", null));
