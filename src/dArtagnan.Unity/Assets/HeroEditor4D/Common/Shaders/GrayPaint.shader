    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ToGrayscale(float3 pixel, float3 color, float saturation, float saturationBound,
                               float colorMultiplier, half inverse)
            {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR
            {
                ...
                float4 node_3046_p = lerp(float4(float4(_MainTex_var.rgb, 0.0).zy, node_3046_k.wz),
                             float4(float4(_MainTex_var.rgb, 0.0).yz, node_3046_k.xy),
                             step(float4(_MainTex_var.rgb, 0.0).z, float4(_MainTex_var.rgb, 0.0).y));
                float4 node_3046_q = lerp(float4(node_3046_p.xyw, float4(_MainTex_var.rgb, 0.0).x),
                                               float4(float4(_MainTex_var.rgb, 0.0).x, node_3046_p.yzx),
                                               step(node_3046_p.x, float4(_MainTex_var.rgb, 0.0).x));
                float node_3046_d = node_3046_q.x - min(node_3046_q.w, node_3046_q.y);
                float node_3046_e = 1.0e-10;
                float3 node_3046 = float3(
                    abs(node_3046_q.z + (node_3046_q.w - node_3046_q.y) / (6.0 * node_3046_d + node_3046_e)),
                    node_3046_d / (node_3046_q.x + node_3046_e), node_3046_q.x);;
                float3 node_9613 = ToGrayscale(_MainTex_var.rgb, i.vertexColor.rgb, node_3046.g, _SaturationBound,
                     _ColorMultiplier, _Inverse);
