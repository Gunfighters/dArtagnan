...
namespace Assets.HeroEditor4D.InventorySystem.Scripts
{
    ...
    public class ItemCollection : ScriptableObject
    {
        ...
        [Header("Main")] public List<SpriteCollection> SpriteCollections;

        public List<IconCollection> IconCollections;
        public List<ItemParams> Items;

        [Header("Extra")] public Sprite BackgroundBlue;
        ...
        public ItemParams GetItemParams(Item item)
        {
            ...
            if (itemParams == null) throw new Exception($"Item params not found: {item.Id}");
        ...
        {
            if (_itemSprites == null) _itemSprites = CacheSprites();

            var itemParams = GetItemParams(item);

            if (itemParams.SpriteId == null) return null;

            if (_itemSprites.ContainsKey(itemParams.SpriteId)) return _itemSprites[itemParams.SpriteId];
        ...
        {
            if (_itemIcons == null) _itemIcons = CacheIcons();

            var itemParams = GetItemParams(item);

            if (itemParams.IconId == null) return null;

            if (_itemIcons.ContainsKey(itemParams.IconId)) return _itemIcons[itemParams.IconId];
        ...
        public Sprite FindSprite(string spriteId)
        {
            ...
            if (spriteId != null && _itemSprites.ContainsKey(spriteId)) return _itemSprites[spriteId].Sprite;
        ...
        public Sprite FindIcon(string iconId)
        {
            ...
            if (iconId != null && _itemIcons.ContainsKey(iconId)) return _itemIcons[iconId].Sprite;
