...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    {
        protected static readonly List<PropertyId> Sorting = new()
        {
            PropertyId.Damage,
        ...
        public virtual void Initialize(Item item, int price, bool trader = false)
        {
            ...
            foreach (var t in item.Params.Tags) main.Add(t);
            ...
            if (item.Modifier != null) dict.Add("ItemInfo.Modifier", $"{item.Modifier.Id} [{item.Modifier.Level}]");

            var props = item.Params.Properties.ToList().OrderBy(i =>
            {
                var index = Sorting.IndexOf(i.Id);
            ...
            if (Price && item.Params.Type != ItemType.Currency) dict.Add("ItemInfo.Price", $"{item.Params.Price} gold");
