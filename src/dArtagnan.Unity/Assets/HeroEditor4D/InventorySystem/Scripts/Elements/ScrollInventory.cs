...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    public class ScrollInventory : ItemContainer
    {
        ...
        private static readonly List<ItemType> TypePriority = new()
        {
            ItemType.Currency,
            ItemType.Container,
            ItemType.Booster,
        ...
        private readonly List<InventoryItem>
            _itemInstances = new(); // Reusing instances to reduce Instantiate() calls.
        ...
        public void Refresh(Item selected, bool force)
        {
            ...
            if (FilterFunc != null) items.RemoveAll(i => !FilterFunc(i));
            ...
            if (!force && _hash == GetHash(items))
            {
                ...
                for (var i = 0; i < items.Count; i++) instances[i].Initialize(items[i]);
            ...
            switch (Grid.constraint)
            {
                ...
                {
                    var height =
                        Mathf.FloorToInt((ScrollRect.GetComponent<RectTransform>().rect.height + Grid.spacing.y) /
                ...
                {
                    var width = Mathf.FloorToInt(
                        (ScrollRect.GetComponent<RectTransform>().rect.width + Grid.spacing.x) /
        ...
        {
            return string.Join(":", items.Select(i => $"{i.Id}.{i.Modifier?.Id}.{i.Modifier?.Level}.{i.Count}"))
                .GetHashCode();
        }
    }
}