...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    public class Equipment : ItemContainer
    {
        ...
        private void Reset()
        {
            foreach (var inventoryItem in InventoryItems) Destroy(inventoryItem.gameObject);
        ...
        public void SetBagSize(int size)
        {
            ...
            var supplies = GetComponentsInChildren<ItemSlot>(true).Where(i => i.Types.Contains(ItemType.Supply))
                .ToList();

            for (var i = 0; i < supplies.Count; i++) supplies[i].Locked = i >= size;
        }
        ...
        private Item FindItem(ItemSlot slot)
        {
            ...
            {
                var copy =
                    Items.SingleOrDefault(i => i.Params.Type == ItemType.Weapon && (i.IsTwoHanded || i.IsFirearm));
