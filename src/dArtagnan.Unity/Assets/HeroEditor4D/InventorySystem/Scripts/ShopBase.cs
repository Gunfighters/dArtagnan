...
namespace Assets.HeroEditor4D.InventorySystem.Scripts
{
    ...
    public class ShopBase : ItemWorkspace
    {
        ...
        public void Start()
        {
            if (ExampleInitialize) TestInitialize();
        }
        ...
        public virtual int GetPrice(Item item) // Override this function to fit your needs!
        {
            ...
            if (trader) price *= GetTraderMarkup(item);
        ...
        public void RegisterCallbacks()
        {
            ...
            InventoryItem.OnRightClick = InventoryItem.OnDoubleClick = item =>
            {
                SelectItem(item);
                if (TraderInventory.Items.Contains(item)) Buy();
                else Sell();
            };
        }
        ...
        {
            var inventory = new List<Item> { new(CurrencyId, 10000) };
            ...
            if (!TraderInventory.SelectAny() && !PlayerInventory.SelectAny()) ItemInfo.Reset();
        }
        ...
        public void Buy()
        {
            ...
            MoveItem(item, TraderInventory, PlayerInventory, Amount, CurrencyId);
        ...
        public void Sell()
        {
            ...
            AddMoney(PlayerInventory, price, CurrencyId);
            AddMoney(TraderInventory, -price, CurrencyId);
            MoveItem(SelectedItem, PlayerInventory, TraderInventory, Amount, CurrencyId);
            AudioSource.PlayOneShot(TradeSound, SfxVolume);
        }
        ...
        public override void Refresh()
        {
            ...
            else
            {
                if (TraderInventory.Items.Contains(SelectedItem))
                    ...
                    InitBuy();
                else if (PlayerInventory.Items.Any(i => i.Hash == SelectedItem.Hash)) InitSell();
            }
        ...
        {
            BuyButton.SetActive(SelectedItem.Params.Type != ItemType.Currency && SelectedItem.Count > 0 &&
                                !SelectedItem.Params.Tags.Contains(ItemTag.NotForSale) &&
                                !SelectedItem.Params.Tags.Contains(ItemTag.Quest) && CanBuy(SelectedItem));
            SellButton.SetActive(false);
        ...
        {
            BuyButton.SetActive(false);
            SellButton.SetActive(SelectedItem.Count > 0 && !SelectedItem.Params.Tags.Contains(ItemTag.NotForSale) &&
                                 !SelectedItem.Params.Tags.Contains(ItemTag.Quest) && SelectedItem.Id != CurrencyId &&
                                 CanSell(SelectedItem));
        ...
        private static void AddMoney(ItemContainer inventory, int value, string currencyId)
        {
            ...
            else
            {
                ...
                if (currency.Count == 0) inventory.Items.Remove(currency);
            }
        }
    }
}