    ...
    public class CharacterInventorySetup
    {
        public static void Setup(Character character, List<Item> equipped, CharacterAppearance appearance)
        {
            if (!appearance.Underwear.IsEmpty())
                character.Underwear =
                    character.SpriteCollection.Armor.Single(i => i.Name == appearance.Underwear).Sprites;

            character.UnderwearColor = appearance.UnderwearColor;
            character.ShowHelmet = appearance.ShowHelmet;
            appearance.Setup(character, false);
        ...
        public static void Setup(Character character, List<Item> equipped)
        {
                ...
                try
                {
                    ...
                    switch (item.Params.Type)
                    {
                            ...
                            switch (item.Params.Class)
                            {
                                ...
                                case ItemClass.Firearm:
                                    ...
                                    character.SecondaryWeapon =
                                        spriteCollection.Firearm1H.FindSprites(item.Params.SpriteId)[1];
                                    break;
                                default:
                                    if (equipped.Any(i => i.IsFirearm))
                                    {
                                        ...
                                        character.PrimaryWeapon =
                                            spriteCollection.MeleeWeapon1H.FindSprite(item.Params.SpriteId);
                                    }
                                    else
                                    {
                                        character.WeaponType = item.Params.Tags.Contains(ItemTag.TwoHanded)
                                            ? WeaponType.Melee2H
                                            : WeaponType.Melee1H;
                                        character.PrimaryWeapon =
                                            (character.WeaponType == WeaponType.Melee1H
                                                ? spriteCollection.MeleeWeapon1H
                                                : spriteCollection.MeleeWeapon2H).FindSprite(item.Params.SpriteId);
                                    }
                                ...
                                case ItemClass.Bomb:
                                    ...
                                    character.PrimaryWeapon =
                                        spriteCollection.Throwable.FindSprite(item.Params.SpriteId);
                                    break;
                            }
                        ...
                        case ItemType.Helmet:
                            ...
                            break;
                        case ItemType.Vest:
                            character.Equip(spriteCollection.Armor.Single(i => i.Id == item.Params.SpriteId),
                                EquipmentPart.Vest, Color.white);
                            break;
                        case ItemType.Bracers:
                            character.Equip(spriteCollection.Armor.Single(i => i.Id == item.Params.SpriteId),
                                EquipmentPart.Bracers, Color.white);
                            break;
                        case ItemType.Leggings:
                            character.Equip(spriteCollection.Armor.Single(i => i.Id == item.Params.SpriteId),
                                EquipmentPart.Leggings, Color.white);
                            break;
                    }
                }
                catch (Exception e)
            ...
            foreach (var part in new[] { ItemType.Vest, ItemType.Bracers, ItemType.Leggings })
                ...
                {
                    var entry = character.SpriteCollection.Armor.Single(i =>
                        i.Sprites.Contains(character.Underwear[0]));
