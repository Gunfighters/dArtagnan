...
namespace Assets.HeroEditor4D.InventorySystem.Scripts
{
    ...
    public class InventoryBase : ItemWorkspace
    {
        ...
        private List<Item> MaterialList => SelectedItem.Params.FindProperty(PropertyId.Materials).Value.Split(',')
            .Select(i => i.Split(':')).Select(i => new Item(i[0], int.Parse(i[1]))).ToList();
        ...
        public void Start()
        {
            if (InitializeExample) TestInitialize();
        }
        ...
        public void Initialize(ref List<Item> inventory, ref List<Item> equipped, int bagSize, Action onRefresh)
        {
            ...
            if (!Equipment.SelectAny() && !PlayerInventory.SelectAny()) ItemInfo.Reset();
        }
        ...
        private void QuickAction(Item item)
        {
            ...
            if (Equipment.Items.Contains(item))
                Remove();
            else if (CanEquipSelectedItem()) Equip();
        }
        ...
        public void Equip()
        {
            ...
            if (equipped.Any()) AutoRemove(equipped, Equipment.Slots.Count(i => i.Supports(SelectedItem)));
        ...
        public void Use(AudioClip sound)
        {
            if (SelectedItem.Count == 1)
            {
                ...
                if (SelectedItem == null)
                {
                    ...
                    if (SelectedItem != null) Equipment.Refresh(SelectedItem);
                }
                else
                {
                    PlayerInventory.Refresh(SelectedItem);
            ...
            if (sound != null) AudioSource.PlayOneShot(sound, SfxVolume);
        }

        public Item Assemble()
        {
            if (SelectedItem != null && SelectedItem.Params.Type == ItemType.Fragment && SelectedItem.Count >=
                SelectedItem.Params.FindProperty(PropertyId.Fragments).ValueInt)
            {
        ...
        public override void Refresh()
        {
            ...
            AssembleButton.SetActive(SelectedItem != null && SelectedItem.Params.Type == ItemType.Fragment &&
                                     SelectedItem.Count >=
                                     SelectedItem.Params.FindProperty(PropertyId.Fragments).ValueInt);
            ...
            if (receipt)
            {
                if (LearnButton == null)
                {
                    var materialSelected = !PlayerInventory.Items.Contains(SelectedItem) &&
                                           !Equipment.Items.Contains(SelectedItem);
        ...
        private void AutoRemove(List<Item> items, int max = 1)
        {
            ...
            foreach (var p in items) sum += p.Count;

            if (sum == max)
                MoveItemSilent(items.LastOrDefault(i => i.Id != SelectedItem.Id) ?? items.Last(), Equipment,
                    PlayerInventory);
        }
    }
}